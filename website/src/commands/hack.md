...
After running `git hack branch-3`, our workspace contains these branches:
...
it also [syncs](sync.md) the main branch to ensure you develop on top of the
current state of the repository.
If the workspace is not clean
(contains uncommitted changes),
`git town hack` does not perform this sync to let you commit your open changes.

### Upstream remote

If the repository contains a remote called `upstream`,
it also syncs the main branch with its upstream counterpart.
...
Enables or disables [automatic syncing](../preferences/auto-sync.md)
...
`git town hack` creates a remote tracking branch
and optionally a [proposal](propose.md) for the new feature branch.
